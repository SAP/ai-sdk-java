...
class AiCoreOpenAiTest {
  ...
  //                + " systems from the 1960s to today. Include subsections, citations, and
  // concrete"
  ...
  //    final var response = service.createResponseWithTools("What is the weather in Berlin
  // today?");
  //    assertThat(response).isNotNull();
  //    assertThat(response.output()).isNotEmpty();
  ...
  //    final var response =
  //        service.createStructuredResponse("Extract the name and age from: John is 30 years
  // old.");
  //    assertThat(response).isNotNull();
  //    final var message =
  //        response.output().stream().filter(ResponseOutputItem::isMessage).findFirst();
  ...
  //
  //    // Second call with the same key — cache hit is server-side and not guaranteed in tests, so
  // we
