...
class SpeechToTextRealtimeClient extends WSSOpenAIRealtimeClient implements AudioInputChannel {
    ...
    public SpeechToTextRealtimeClient(String url, Map<String, String> httpHeaders, TextOutputChannel outputConsumer) {
    ...
    protected void onResponse(String eventType, JsonNode event) {
        if ("response.output_audio_transcript.done".equals(eventType)) {
            this.outputConsumer
                    .outputText(event.has("transcript") ? event.get("transcript").asText() : "", Boolean.TRUE);
        } else {
                                        ...
                                        RealtimeTranscriptionSessionAudioInput.builder()
                                            ...
                                            .transcription(
                                                AudioTranscription.builder()
                                                    .model(AudioTranscription.Model.GPT_4O_TRANSCRIBE)
                                                .build())
                                            .build())
                                    .build())
                            .build())
